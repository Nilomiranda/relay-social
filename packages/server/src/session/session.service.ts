...
export class SessionService {
    ...
    async login(login: LoginInput): Promise<Session> {
        ...
        const previousSession = await this.repo.findOne({ where: { user: { id: user.id } }, relations: ['user'] });
    ...
    async validateSession(token: string): Promise<User> {
        const session = await this.repo.findOne({ where: { token, status: SessionStatus.VALID }, relations: ['user'] });
