...
export class PostsResolver {
  ...
  @Mutation(type => Post)
  async newPost(@Args('newPostData') newPostData: PostInput, @CurrentUser() user: User): Promise<Post> {
  ...
  async post(@Args('id') id: number) {
    const post = await this.postsService.repo.findOne(id, { relations: ['user'] });
